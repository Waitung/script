...
check_linux(){
    if [ $(uname) != 'Linux' ]; then
        echo "错误：这不是 Linux！请在 Debian 10.5 x64 上运行此脚本！"
        exit 1
    fi
}
...
check_machine(){
    if [ $(uname -m) != 'x86_64' -a $(uname -m) != 'amd64' ]; then
        echo "错误：这不是 amd64！请在 Debian 10.5 x64 上运行此脚本！"
        exit 1
    fi
}

check_os(){
    if [[ -z $(grep -i 'Debian GNU/Linux 10' /etc/os-release) ]]; then
        echo "错误：这不是 Debian！请在 Debian 10.5 x64 上运行此脚本！"
        exit 1
    fi
}
...
check_script(){
    if [ ! -d ~/sh ]; then
        mkdir ~/sh
        curl -Ls ${url_repo}/aliyunlite-debian10.5/menu.sh -o ~/sh/menu.sh
...
run(){
    while [ -n "$1" ]; do
        if [ ${choose} -eq 0 ]; then
            if [ -e ~/sh/$1.sh ]; then curl -Ls ${url_repo}/aliyunlite-debian10.5/$1.sh -o ~/sh/$1.sh; echo "$1.sh 更新完毕"; fi
        else
            if [ ! -e ~/sh/$1.sh ]; then curl -Ls ${url_repo}/aliyunlite-debian10.5/$1.sh -o ~/sh/$1.sh; chmod 755 ~/sh/$1.sh; fi
            ~/sh/$1.sh
        fi
        shift
    done
