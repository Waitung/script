...
get_version_filebrowser(){
    ...
    new_version="${filebrowser_tag_name}"
...
get_version_cloudreve(){
    ...
    if [[ -z $(systemctl status cloudreve | grep 'active (running)') ]]; then
        old_version="Cloudreve 未正确运行，无法获取版本号"
    else
        old_version=v`curl -s --unix-socket /run/cloudreve.sock http://localhost/api/v3/site/ping | cut -d\" -f6`
    fi
    echo -e "\n===== Cloudreve =====\n"
}
